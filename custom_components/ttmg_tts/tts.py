...
class BaseGoogleCloudProvider:
    ...
    async def _async_get_tts_audio(
        ...
        #   async with session.post('http://192.168.201.10:8888/preload', json={"text": message}) as resp:
        #       if resp.status != 200:
        #           # Handle error
        #           pass
...
class GoogleCloudTTSEntity(BaseGoogleCloudProvider, TextToSpeechEntity):
    ...
    def __init__(
        ...
        # self._attr_default_language = language  # Set the default language attribute
        # self._language = language  # Initialize the _language attribute
